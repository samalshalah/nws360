  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: `${data.sourcesUpdated + (data.activationUpdated || 0)} source update(s). ${data.deletedArticles} ${data.deleteAllArticles ? "" : "old "}article(s) deleted. ${data.totalNewArticles} new article(s) fetched.`,
      });
    },
    onError: (error) => {
