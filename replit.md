...
- **AI Analysis Pipeline**: A decoupled pipeline where ingestion stores articles as `aiAnalysisStatus='pending'` and enqueues `ANALYZE_ARTICLE` jobs via the processing queue (server/processing-queue.ts). AI analysis runs asynchronously through a central scheduler (`server/ai/ai-scheduler.ts`) with 5-second ticks, 10-job/tick global rate limit, per-tenant budget enforcement (daily token + job limits), and round-robin fair-share allocation across tenants to prevent noisy-neighbor starvation. Jobs transition queued → scheduled → running → completed/failed with atomic status guards. Pre-execution budget re-check prevents over-spending within a single tick. Budget-blocked jobs auto-recover when budget resets. The pipeline applies AI for sentiment, keywords, topics, summary, category, and country detection. An extended AI Intelligence layer (server/ai-intelligence.ts) performs deep analysis: entity extraction, story clustering, narrative comparison, event detection, daily briefs, trend predictions, and a conversational insight assistant.
...
- **Enterprise RBAC & Access Control**: 4-tier role hierarchy (SYSTEM_ADMIN → CLIENT_ADMIN → CLIENT_USER → READONLY_USER) defined in shared/schema.ts as `SYSTEM_ROLES`. Central `resolveTenantContext()` function resolves tenant context with impersonation and admin tenant-dropdown support. Route protection middleware: `requireRole()`, `requireSystemAdmin()`, `requireTenantAccess()`, `requireWriteAccess()`. All routes use `isSystemAdmin()` helpers instead of direct role string comparisons. Capabilities endpoint `GET /api/auth/capabilities` returns structured role-based permissions (feeds, analytics, intelligence, sources, users, billing, systemAdmin, collaboration, integrations, settings, exports, readOnly, executive, knowledgeMemory, predictiveIntelligence) plus tenantName and impersonatingUsername for display. Granular permission system with 23 permission codes across 8 resource categories. 4 default permission groups (Platform Admin, Organization Admin, Analyst, Viewer). Frontend uses `usePermissions()` hook with `canAccess()` for capability-based UI rendering. Admin impersonation via `/api/admin/impersonate/*` endpoints with full audit logging. Tenant selection via `POST /api/admin/select-tenant` for SYSTEM_ADMIN.
- **Dynamic Sidebar Navigation**: Sidebar menu is generated entirely from `/api/auth/capabilities` via `buildMenuStructure()` → `filterMenuByCapabilities()` pipeline (client/src/components/layout/Sidebar.tsx). Menu groups: Core, Analytics, Intelligence, Sources, Collaboration, Integrations, Client Management, Billing, System Monitoring, Executive, Help. Groups with no visible items are automatically hidden. SYSTEM_ADMIN gets tenant switcher dropdown. Impersonation shows "Viewing as: Username (TenantName)" indicator. Zero hardcoded role checks in the sidebar — all visibility driven by `canAccess()` capability checks. MobileBottomNav also filtered by capabilities.
